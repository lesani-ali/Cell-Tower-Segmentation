...
class SegmentationModel:
    def __init__(self, config: SegmentationConfig):
        ...
        """
        sam = sam_model_registry[config.model_type](checkpoint=config.ckpt).to(
    ...
    def __call__(self, image: Image.Image, prompts: np.ndarray = None) -> np.ndarray:
        return self.predict(image, prompts)

    def predict(self, image: Image.Image, prompts: np.ndarray = None) -> np.ndarray:
        """
        ...
        result_masks = []
        for box in prompts:
            masks, scores, logits = self.model.predict(box=box, multimask_output=True)
